...
const ApplianceControl = () => {
  ...
  const fetchAppliances = async () => {
    try {
      ...
      const response = await axios.get(`${API_URL}/appliance/list`, {
  ...
  const toggleApplianceStatus = async (id) => {
    try {
      ...
      setAppliances(appliances.map((a) => (a.id === id ? { ...a, status: a.status === "on" ? "off" : "on" } : a)))

      // Call API to toggle appliance
      await axios.post(
        `${API_URL}/appliance/${id}/toggle`,
        {
      ...
      setSuccess(`${appliance.name} turned ${appliance.status === "on" ? "off" : "on"} successfully`)
      setTimeout(() => setSuccess(null), 3000)
    } catch (err) {
  ...
  const handleAddAppliance = async () => {
    try {
      ...
      // Call API to add appliance
      const response = await axios.post(`${API_URL}/appliance/add`, formData, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      })

      // Update appliances list
      setAppliances([...appliances, response.data.appliance])
  ...
  const handleEditAppliance = async () => {
    try {
      ...
      // Call API to update appliance
      await axios.put(`${API_URL}/appliance/${currentAppliance.id}/update`, formData, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      })

      // Update appliances list
      setAppliances(appliances.map((a) => (a.id === currentAppliance.id ? { ...a, ...formData } : a)))
  ...
  const handleDeleteAppliance = async (id) => {
    ...
    try {
      ...
      // Call API to delete appliance
      await axios.delete(`${API_URL}/appliance/${id}/delete`, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      })
  ...
  const calculateEnergyUsage = (appliance) => {
    ...
    const usageHours =
      appliance.type === "refrigerator" ? 24 : appliance.type === "lighting" ? 6 : appliance.type === "hvac" ? 8 : 4
  ...
  const filteredAppliances = appliances.filter(
    (appliance) =>
      ...
      (appliance.location && appliance.location.toLowerCase().includes(searchTerm.toLowerCase())) ||
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
          <div>
            ...
            <p className="text-gray-600">Manage and control your household appliances</p>
          </div>
          ...
          <div className="bg-gradient-to-br from-orange-50 to-amber-50 border border-orange-100 rounded-lg p-4 shadow-sm">
            <div className="flex items-start">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-orange-700">{appliances.length}</p>
                <p className="text-xs text-gray-600">{appliances.filter((a) => a.is_smart).length} smart devices</p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-start">
              ...
              <div>
                ...
                <p className="text-xs text-gray-600">
                  {((appliances.filter((a) => a.status === "on").length / appliances.length) * 100).toFixed(0)}% of
                  total
                </p>
              </div>
            </div>
          ...
          <div className="bg-gradient-to-br from-blue-50 to-sky-50 border border-blue-100 rounded-lg p-4 shadow-sm">
            <div className="flex items-start">
              ...
              <div>
                ...
                <p className="text-xs text-gray-600">
                  Total capacity: {appliances.reduce((sum, a) => sum + a.power_rating / 1000, 0).toFixed(2)} kW
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-purple-50 to-indigo-50 border border-purple-100 rounded-lg p-4 shadow-sm">
            <div className="flex items-start">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-purple-700">{appliances.filter((a) => a.is_smart).length}</p>
                <p className="text-xs text-gray-600">Automated energy management</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm mb-6">
          <div className="p-4 border-b border-gray-200">
            <h2 className="text-lg font-semibold text-gray-800">Your Appliances</h2>
          </div>
                ...
                {filteredAppliances.length > 0 ? (
                    ...
                    <tr key={appliance.id}>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <span className="text-xl mr-2">{getApplianceTypeIcon(appliance.type)}</span>
                          <div>
                            <div className="text-sm font-medium text-gray-900">{appliance.name}</div>
                            <div className="text-sm text-gray-500">
                              {appliance.is_smart ? "Smart Device" : "Standard Device"}
                            </div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{appliance.location || "-"}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{appliance.power_rating} W</td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                            appliance.status === "on" ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"
                          }`}
                        >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex space-x-2">
                          <button
                            onClick={() => toggleApplianceStatus(appliance.id)}
                            className={`p-1 rounded ${
                              appliance.status === "on"
                                ? "text-red-600 hover:bg-red-100"
                                : "text-green-600 hover:bg-green-100"
                            }`}
                            title={appliance.status === "on" ? "Turn Off" : "Turn On"}
                          >
                            <FaPowerOff />
                          </button>
                          <button
                  ...
                  <tr>
                    <td colSpan="7" className="px-6 py-4 text-center text-gray-500">
                      No appliances found. Add your first appliance to get started.
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            <div className="p-4 border-b border-gray-200">
              <h3 className="text-lg font-semibold text-gray-800">Add New Appliance</h3>
            </div>
            <div className="p-4">
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Power Rating (Watts)</label>
                  <input
                    type="number"
      ...
      {showEditModal && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            <div className="p-4 border-b border-gray-200">
              <h3 className="text-lg font-semibold text-gray-800">Edit Appliance</h3>
            </div>
            <div className="p-4">
              <div className="space-y-4">
                <div>
